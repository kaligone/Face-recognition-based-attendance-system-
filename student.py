...
class Student:
    def __init__(self,root):
        ...
        studentID_entry=ttk.Entry(class_Student_frame,textvariable=self.va_std_id,font=("times new roman",12,"bold"))
        ...
        studentName_entry=ttk.Entry(class_Student_frame,textvariable=self.var_std_name,font=("times new roman",12,"bold"))
        ...
        gender_combo["values"]=("SLECT GENDER","MALE","FEMALE","OTHER")
        ...
        save_btn=Button(btn_frame,text="SAVE",command=self.add_data,width=11,font=("times new roman",12,"bold"),bg="chocolate")
        save_btn.grid(row=0,column=0,padx=2)

        delete_btn=Button(btn_frame,text="DELETE",command=self.delete_data,width=11,font=("times new roman",12,"bold"),bg="chocolate")
        delete_btn.grid(row=0,column=1,padx=2)

        update_btn=Button(btn_frame,text="UPDATE",command=self.update_data,width=11,font=("times new roman",12,"bold"),bg="chocolate")
        update_btn.grid(row=0,column=2,padx=2)

        reset_btn=Button(btn_frame,text="RESET",command=self.reset_data,width=11,font=("times new roman",12,"bold"),bg="chocolate")
        ...
        take_photo_btn=Button(btn_frame_1,command=self.generate_dataset,text="TAKE PHOTO SAMPLE",width=22,font=("times new roman",12,"bold"),bg="chocolate")
        ...
        self.student_table=ttk.Treeview(table_frame,column=("dep","course","sem","id","name","div","roll","gender","dob","email","phone","address","photo"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
    ...
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.va_std_id.get()=="":
            ...
            try:
                ...
                self.fetch_data()
                conn.close()
                messagebox.showerror("Success","Student details are added sucessfully",parent=self.root)
        ...
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.va_std_id.get()=="":
            ...
            try:
                ...
                if Update>0:
                     conn=mysql.connector.connect(host="localhost",user="root",password="",database="face_recognizer")
                     my_cursor=conn.cursor()
                     my_cursor.execute("UPDATE student SET Dep=%s,course=%s,Semester=%s,Name=%s,Division=%s,Roll=%s,Gender=%s,DOb=%s,Email=%s,Phone=%s,Address=%s,PhotoSample=%s WHERE Student_id=%s",(
                                                                                                                                                                                                        self.var_dep.get(),
                                                                                                                                                                                                        self.var_course.get(),
                                                                                                                                                                                                        self.var_semester.get(),
                    ...
                    if not Update:
                        return
                messagebox.showinfo("Sucess","Student details successfully update",parent=self.root)
                conn.commit()
                self.fetch_data()
                conn.close()
            except Exception as es:
    ...
    def delete_data(self):
        if self.va_std_id.get()=="":
            ...
            try:
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Delete","Sucessfully Delete",parent=self.root)
            except Exception as es:
                messagebox.showerror("Error",f"Due To:{str(es)}",parent=self.root)
